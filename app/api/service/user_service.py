...
class UserService(SqlAlchemyRepository):
    ...
    def check_if_email_already_exists(self, db: Session, email: str, user_obj: User) -> bool:
    ...
    def deactivate_role(self, db: Session, role: str,  user_obj: User) -> User:
        if (deleted_role := self.get_role(db, role)) is not None:        
            user_obj.roles = [user_role for user_role in user_obj.roles if user_role.name != deleted_role.name]
        return user_obj
    
    #TODO expirar sessao depois de atualizar campos de alteracao de tipo de usuario
    def update_user(self, db: Session, obj_in: schemas.UserUpdate, user_obj: User) -> User:
        ...
        if obj_in.category or obj_in.is_manager:
            ...
            if manager is not None:
                ...
                if obj_in.category:
                    if manager.active is True and manager.manager_category_id != obj_in.category:
                        manager.manager_category_id = obj_in.category
                        db.add(manager)
                
                if obj_in.is_manager is True and obj_in.category is None and manager.active is False:
                    manager.active = True
                    db.add(manager)
            ...
            if manager is None and obj_in.is_manager is not False and obj_in.category:
                user_obj = self.activate_role(db, "manager", user_obj)
                manager = Manager(user_id=user_obj.user_id, manager_category_id=obj_in.category)
                db.add(manager)
